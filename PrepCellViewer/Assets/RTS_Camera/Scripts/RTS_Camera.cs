...
namespace RTS_Cam
{
    ...
    public class RTS_Camera : MonoBehaviour
    {
        ...
        private void CameraUpdate()
        {
            ...
            Center.transform.LookAt(m_Transform);
            HeightCalculation();
            Rotation();
            RotateAround();
            LimitPosition();

            if (Input.GetButton("But1"))
            {
                Quaternion tmp = Quaternion.LookRotation(Center.position - m_Transform.position, Center.up);

                Quaternion Looking = Quaternion.Slerp(m_Transform.rotation, tmp, Time.deltaTime * 10);
                m_Transform.rotation = Looking;
            }
        }
        ...
        private void HeightCalculation()
        {
            ...
            if(useScrollwheelZooming && !Input.GetButton("But1"))
                zoomPos += ScrollWheel * Time.deltaTime * scrollWheelZoomingSensitivity;
            if (useKeyboardZooming)
            {
                zoomPos += ZoomDirection * Time.deltaTime * keyboardZoomingSensitivity;
                m_Transform.Translate(0, zoomPos, 0);
            }
            ...
            {
                zoomPos += ScrollWheel * Time.deltaTime * scrollWheelZoomingSensitivity;
                m_Transform.Translate(0, 0 ,zoomPos);
            }
        ...
        private void Rotation()
        {
            ...
            if (useMouseRotation && Input.GetKey(mouseRotationKey) && !Input.GetKey("left alt"))
                m_Transform.Rotate(Vector3.up, MouseAxis.x * Time.deltaTime * mouseRotationSpeed, Space.Self);

            if (useMouseRotation && Input.GetKey(mouseRotationKey) && !Input.GetKey("left alt"))
                m_Transform.Rotate(Vector3.left, -MouseAxis.y * Time.deltaTime * mouseRotationSpeed, Space.Self);
            
            m_Transform.transform.eulerAngles = new Vector3(this.transform.eulerAngles.x, this.transform.eulerAngles.y, 0);
        }

        private void RotateAround()
        {
            //m_Transform.transform.LookAt(Center);


            if (useMouseRotation && Input.GetKey(mouseRotationKey) && Input.GetButton("But1"))
            { 
                m_Transform.transform.RotateAround(Center.transform.position, Center.transform.up, MouseAxis.x * Time.deltaTime * mouseRotationSpeed);
                m_Transform.transform.LookAt(Center);
            }
            ...
            if (useMouseRotation && Input.GetKey(mouseRotationKey) && Input.GetButton("But1"))
            { 
                m_Transform.transform.RotateAround(Center.transform.position, Center.transform.right, -MouseAxis.y * Time.deltaTime * mouseRotationSpeed);
                m_Transform.transform.LookAt(Center);
            }
            ...
            // dont turn over 80 degrees - causes gliches
            if (m_Transform.transform.rotation.eulerAngles.x < 180)
            {
                if (m_Transform.transform.rotation.eulerAngles.x > 80)
                    m_Transform.transform.RotateAround(Center.transform.position, Center.transform.right, MouseAxis.y * Time.deltaTime * mouseRotationSpeed);
            }
            else
                if (m_Transform.transform.rotation.eulerAngles.x < 280)
                    m_Transform.transform.RotateAround(Center.transform.position, Center.transform.right, MouseAxis.y * Time.deltaTime * mouseRotationSpeed);

            //dont hit floor
            if (m_Transform.transform.position.y < 0.8)
                m_Transform.transform.RotateAround(Center.transform.position, Center.transform.right, MouseAxis.y * Time.deltaTime * mouseRotationSpeed);
